...
class ResNeXtBlock(nn.Module):
    def __init__(self, in_channels, out_channels, d, C, bottleneck = False):
        ...
        stride = 2 if 2*in_channels == out_channels else 1
        ...
        if bottleneck:
            ...
            self.F.add_module('conv1', nn.Conv2d(in_channels, hidden_channels, kernel_size = 3, padding = 1, bias = False))
            self.F.add_module('bn1',nn.BatchNorm2d(hidden_channels))
            self.F.add_module('relu1', nn.ReLU())
            self.F.add_module('conv2', nn.Conv2d(hidden_channels, out_channels, kernel_size = 3, padding = 1, stride = stride, bias = False))
...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, d, C, n, bottleneck = False):
        ...
        for i in range(n):
            name = "block" + str(i)
            if i == 0:
                self.layers.add_module(name, ResNeXtBlock(in_channels, out_channels, d, C, bottleneck = bottleneck))
            else:
                self.layers.add_module(name, ResNeXtBlock(out_channels, out_channels, d, C, bottleneck = bottleneck))
    
    def forward(self, x):
        return self.layers(x)
    
class ResNeXt(nn.Module):
    def __init__(self, n, d, C, num_class = 10, bottleneck = False):
        super(ResNeXt, self).__init__()
        self.conv = nn.Conv2d(3, 64, kernel_size = 3, padding = 1, bias = False)
        self.bn = nn.BatchNorm2d(64)
        self.layer1 = BasicBlock(64,256,d,C,n,bottleneck = bottleneck)
        self.layer2 = BasicBlock(256,512,d,C,n,bottleneck = bottleneck)
        self.layer3 = BasicBlock(512,1024,d,C,n,bottleneck = bottleneck)
