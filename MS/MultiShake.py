...
class MSBlock(nn.Module):
    ...
    def forward(self, x):
        ## train time
        if self.train:
            alpha = np.array([random.random() for _ in range(self.C)])
            alpha = alpha / np.sum(alpha)
            for i in range(self.C):
                if i == 0:
                    out = alpha[i]*self.Blocks[i](x)
                else:
                    out += alpha[i]*self.Blocks[i](x)
            ## identity mapping
            out += self.id_map(x)
...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, d, C, n, bottleneck = False):
        ...
        for i in range(n):
            name = 'layer' + str(i)
            if i == 0:
                self.layers.add_module(name, MSBlock(in_channels, out_channels, d, C, bottleneck = bottleneck))
            else:
                self.layers.add_module(name, MSBlock(out_channels, out_channels, d, C, bottleneck = bottleneck))
...
class MultiShake(nn.Module):
    def __init__(self, d, C, n, num_class, bottleneck = False):
        super(MultiShake, self).__init__()
        self.train = True
